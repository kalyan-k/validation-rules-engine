...
export class VanillaShowcasePage {
  ...
  async goto(pageSlug = ''): Promise<void> {
    const suffix = pageSlug ? `/${pageSlug}` : '';
    await this.page.goto(`${this.baseUrls.vanilla}${suffix}`);
    await waitForApplicationReady(this.page);
  }

  async expectHome(): Promise<void> {
    await expect(this.page.getByRole('heading', { name: 'Policy validation without a UI framework.' })).toBeVisible();
  ...
  async exercisePerformanceForm(): Promise<void> {
    await expect(this.page.getByRole('heading', { name: 'Large core-validated form' })).toBeVisible();
