...
import type { PolicyRegistration, ValidationGroupRegistration, ValidationTarget } from '../validation/types';
...
export const VANILLA_PERFORMANCE_CONFIG_POLICY = 'vanilla-performance-config';
export const VANILLA_PERFORMANCE_CONFIG_GROUP = 'vanillaPerformanceConfig';
export const VANILLA_PERFORMANCE_ALL_GROUP = 'vanillaPerformance';
...
export function createPerformanceScenario(config: PerformanceConfig = DEFAULT_PERFORMANCE_CONFIG): PerformanceScenario {
  ...
  for (let sectionIndex = 0; sectionIndex < normalized.sectionCount; sectionIndex += 1) {
    ...
    const groupName = `vanillaPerformanceSection${sectionIndex}`;
    const policyName = `VanillaPerformanceSection${sectionIndex}`;
    ...
    for (let fieldIndex = 0; fieldIndex < normalized.controlsPerSection; fieldIndex += 1) {
      ...
      model.sections[sectionId]![fieldKey] = emptyValueFor(field);
      fields.push(field);
    }
  ...
  const policyNames = [VANILLA_PERFORMANCE_CONFIG_POLICY, ...sectionPolicies.map(({ name }) => name)];
  const groups: ValidationGroupRegistration[] = [
    {
      name: VANILLA_PERFORMANCE_CONFIG_GROUP,
      policies: [VANILLA_PERFORMANCE_CONFIG_POLICY],
      formGroups: ['config'],
      fields: CONFIG_FIELDS
    },
    ...sectionGroups,
    {
      name: VANILLA_PERFORMANCE_ALL_GROUP,
  ...
  return {
    ...
    model,
    sections,
    policies: [{ name: VANILLA_PERFORMANCE_CONFIG_POLICY, policy: configPolicy }, ...sectionPolicies],
    policyNames,
    groups,
    metrics: {
